#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build-ios.sh — Build and install diag_tool on iOS (FREE Apple ID)
#
...
# ─────────────────────────────────────────────────────────────────────────────
  ...
  cd "$BUILD_DIR"
  zip -qr "$IPA_DIR/diag_tool.ipa" Payload
  rm -rf "$PAYLOAD_DIR"
  cd "$ROOT"

  IPA_FILE="$IPA_DIR/diag_tool.ipa"
  ...
  echo "    3. Open AltStore on iPhone → My Apps → ＋ → select diag_tool.ipa"
  ...
  echo "    2. Connect iPhone via USB, drag diag_tool.ipa into Sideloadly."
