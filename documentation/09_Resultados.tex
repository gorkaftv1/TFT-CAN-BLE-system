...
% REVISAR [MÉTRICAS]: extraer cifras reales de la BD SQLite para respaldar esta
%   sección (timestamps monotonic_ts/wall_ts de la tabla samples permiten calcular
%   periodos e intervalos reales; tabla commands tiene request/response para contar
%   transacciones). Sustituir rangos estimados por medias/percentiles medidos.
...
En el modo de monitorización continua, el periodo de ciclo medido escala con el número de PIDs solicitados en cada vuelta: con un conjunto reducido de 5--6 PIDs el periodo se aproxima al objetivo de 500\,ms (mediana de 516\,ms con 24 PIDs en una sesión y de 560\,ms con 6 PIDs en otra), mientras que con conjuntos amplios de 18--20 PIDs el ciclo se sitúa en torno a los 600--700\,ms (mediana de 703\,ms con 18 PIDs), por el coste acumulado de las transacciones ISO-TP secuenciales. En todos los casos la dispersión fue baja (rango intercuartílico inferior a 25\,ms en las sesiones deterministas) y el sistema mantuvo una tasa de actualización estable sin pérdidas de muestras. El reenvío de las muestras del hilo de monitorización al cliente BLE no presentó pérdidas ni saturación en ninguno de los ensayos.

\begin{table}[!htbp]
\caption{Cadencia real de la monitorización continua medida sobre la base de datos SQLite (intervalo entre muestras consecutivas por PID).}
\label{tab:cadencia}
\centering
\renewcommand{\arraystretch}{1.2}
