...
\centering
\includegraphics[height=9.5cm]{Ilustraciones/cuadro_konwei.png}
\subcaption{Cuadro de instrumentos (lectura de PIDs).}
\label{fig:ref_panel}
\end{minipage}\hfill
\begin{minipage}[t]{0.42\textwidth}
\centering
\includegraphics[height=9.5cm]{Ilustraciones/dtc_konwei.PNG}
\subcaption{Lista de DTCs.}
\label{fig:ref_dtcs}
\end{minipage}
\caption{Herramienta comercial de referencia (Car Scanner ELM OBD2~\cite{CarScanner_app}): cuadro de instrumentos (a) y lista de DTCs (b).}
\label{fig:ref_comercial}
\end{figure}


% NOTA: tab:latencias (latencia extremo-a-extremo) NO puede sacarse de la BD
%   (solo hay wall_ts de finalización por comando, sin timestamps por etapa).
%   Esa tabla requiere instrumentar el código en vivo; sigue marcada con su TODO.
\section{Validación sobre vehículo real}
\label{sec:resultados_vehiculo_real}

Tras completar la validación sobre el banco de ensayo, el sistema se probó sobre un vehículo real. El vehículo objetivo inicial era un SEAT Ibiza 6J, pero debido a que la plataforma de ese modelo no implementa el protocolo OBD-II sobre el bus CAN en los pines 6 y 14 del puerto OBD, la validación final se realizó sobre un \textbf{Škoda Yeti de 2015}, perteneciente igualmente al grupo VAG y, por tanto, representativo de la misma arquitectura de diagnóstico. La conexión se efectuó mediante un cable OBD-II estándar~\cite{YingtuYou_OBD2} (Figura~\ref{fig:cable_obd}) que nos revela todos los pines del puerto de diagnóstico del vehículo: la Raspberry Pi actuó como interfaz entre el bus CAN del vehículo conectado a través de los pines 6 y 14 (\texttt{CAN\_H} y \texttt{CAN\_L} respectivamente) del puerto OBD-II y la aplicación móvil, manteniendo exactamente el mismo stack de software validado en el banco (\texttt{IsoTpTransport} sobre SocketCAN, servidor BLE GATT con NUS y aplicación React Native).

\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{Ilustraciones/cable_OBD.png}
\caption{Cable OBD-II utilizado para la conexión al puerto de diagnóstico del Škoda Yeti, con los pines expuestos que permiten identificar y conectar directamente \texttt{CAN\_H} (pin~6) y \texttt{CAN\_L} (pin~14).}
\label{fig:cable_obd}
\end{figure}
...
\begin{figure}[!htbp]
\centering
\includegraphics[height=7.5cm]{Ilustraciones/foto_coche_pi.jpeg}
...
\centering
\includegraphics[height=9.5cm]{Ilustraciones/app_session_muestras.jpg}
\subcaption{Muestras de la sesión por sensor.}
\label{fig:skoda_muestras}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[height=9.5cm]{Ilustraciones/app_session_muestra_detalle.jpg}
\subcaption{Detalle de una muestra.}
\label{fig:skoda_muestra_detalle}
\end{minipage}
\caption{Validación sobre el Škoda Yeti: muestras de telemetría agrupadas por sensor (a) y detalle de una muestra concreta con su valor decodificado (b).}
\label{fig:skoda_sesion}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[height=9.5cm]{Ilustraciones/app_session_dtc.jpg}
\caption{Validación sobre el Škoda Yeti: lista de DTCs leídos de la ECU del vehículo.}
\label{fig:skoda_dtc}
\end{figure}

El VIN reconstruido por la herramienta mediante la secuencia ISO-TP multi-frame fue \texttt{TMBJF45LXF6022462} (código de fabricante \texttt{TMB}, correspondiente a Škoda), coincidiendo carácter a carácter con el grabado en el vehículo. La tabla~\ref{tab:skoda_telemetria} compara los rangos de telemetría medidos por la herramienta desarrollada sobre el Škoda Yeti (extraídos de la base de datos SQLite con \texttt{scripts/db\_metrics.py} sobre un total de 1\,627 muestras) con las lecturas obtenidas por la herramienta comercial de referencia (KONWEII junto a Car Scanner ELM OBD2~\cite{KONWEII_scanner, CarScanner_app}) sobre el mismo vehículo.

\begin{table}[!htbp]
\caption{Comparación de la telemetría del Škoda Yeti: herramienta desarrollada (base de datos SQLite) frente a la herramienta comercial de referencia.}
\label{tab:skoda_telemetria}
\centering
\renewcommand{\arraystretch}{1.2}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|l|c|c|c|}
\hline
\textbf{PID} & \textbf{Parámetro} & \textbf{Herramienta desarrollada} & \textbf{Herramienta comercial} & \textbf{Coincide} \\
\hline
\texttt{0x0C} & RPM del motor          & 0 -- 3843\,rpm   & 0 -- 3843\,rpm   & Sí \\
\texttt{0x05} & Temp. refrigerante     & 33 -- 72\,°C     & 33 -- 63\,°C     & Sí$^{*}$ \\
\texttt{0x11} & Posición acelerador    & 12,2 -- 86,7\,\% & 12,2 -- 86,7\,\% & Sí \\
\texttt{0x04} & Carga calculada motor  & 0 -- 55,7\,\%    & 0 -- 55,7\,\%    & Sí \\
\texttt{0x0B} & Presión MAP admisión   & 24 -- 107\,kPa   & 24 -- 107\,kPa   & Sí \\
\texttt{0x42} & Tensión de batería     & 10,4 -- 14,1\,V  & 10,4 -- 14,1\,V  & Sí \\
\texttt{0x43} & Carga absoluta         & 19,2 -- 62,4\,\% & 19,2 -- 62,4\,\% & Sí \\
\texttt{0x33} & Presión barométrica    & 101\,kPa         & 106\,kPa         & $\approx$ \\
\texttt{0x0D} & Velocidad              & 255\,km/h (cte.) & 255\,km/h (cte.) & Sí \\
\hline
\end{tabular}%
}

\vspace{2pt}
{\footnotesize $^{*}$ La prueba con la herramienta comercial fue más breve, por lo que el motor alcanzó una temperatura de refrigerante inferior (63\,°C frente a 72\,°C). El rango es coherente en ambos casos.}
\end{table}

Las lecturas de ambas herramientas resultaron equivalentes en la práctica totalidad de los parámetros. Las únicas diferencias fueron la temperatura de refrigerante (consecuencia de la distinta duración de cada prueba) y la presión barométrica (106\,kPa frente a 101\,kPa, una desviación de unos 5\,kPa atribuible a la tolerancia y calibración del sensor). Esta correspondencia confirma la validez de las lecturas de la herramienta desarrollada frente a un instrumento comercial de referencia.

El valor del PID \texttt{0x0D} (velocidad) se mantuvo constante en 255\,km/h (\texttt{0xFF}) durante toda la captura, valor que la herramienta registró fielmente y que se corresponde con una lectura no válida del sensor de velocidad. Este comportamiento es coherente con los DTC almacenados en la ECU del vehículo, leídos por la herramienta y recogidos en la tabla~\ref{tab:skoda_dtcs}: el código \texttt{P0501} (\textit{rango/rendimiento del sensor de velocidad}) explica directamente la lectura anómala, lo que demuestra que la telemetría y el diagnóstico de averías obtenidos por la herramienta son mutuamente consistentes.

\begin{table}[!htbp]
\caption{DTCs reales leídos por la herramienta de la ECU del Škoda Yeti.}
\label{tab:skoda_dtcs}
\centering
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|c|l|}
\hline
...
\hline
\texttt{P0501} & Circuito del sensor de velocidad del vehículo A — rango/rendimiento \\
\texttt{U0415} & Datos no válidos recibidos del módulo de control del ABS \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[height=10cm]{Ilustraciones/opel_konwei.PNG}
\caption{Herramienta comercial de referencia (Car Scanner ELM OBD2~\cite{CarScanner_app}) sobre el Opel Astra K: no detecta ningún DTC activo.}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[height=10cm]{Ilustraciones/opel_panel.png}
\subcaption{Panel de telemetría.}
\label{fig:opel_panel}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[height=10cm]{Ilustraciones/opel_dtc.png}
\subcaption{Escaneo de DTCs.}
\label{fig:opel_dtc}
\end{minipage}
\caption{Herramienta desarrollada sobre el Opel Astra K: panel de telemetría (a) y escaneo de DTCs (b).}
...
Se midió la latencia round-trip extremo a extremo de una consulta completa, desde el envío del comando JSON por la aplicación móvil hasta la recepción de la respuesta decodificada. Las marcas de tiempo de transmisión (\texttt{ble\_tx}) y de recepción (\texttt{ble\_rx}) registradas por la propia aplicación durante una sesión real sobre el banco de pruebas permiten emparejar cada petición con su respuesta; sobre las 38 transacciones de la sesión, el script \texttt{scripts/latency\_from\_csv.py} calcula los estadísticos recogidos en la tabla~\ref{tab:latencias}.

\begin{table}[H]
\caption{Latencia round-trip extremo a extremo medida sobre 38 transacciones reales.}
\label{tab:latencias}
\centering
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|l|c|}
\hline
\textbf{Estadístico} & \textbf{Latencia (ms)} \\
\hline
Mínimo        & 57 \\
Mediana       & 100 \\
Media         & 128 \\
...
La latencia dominante la introduce la capa BLE (las operaciones \textbf{Write} y \textbf{Notify}), siendo la contribución de CAN+ISO-TP despreciable a las velocidades de bus empleadas. La mediana se mantiene en torno a 100\,ms, coherente con el carácter interactivo de la herramienta. El percentil 95 más elevado (387\,ms) corresponde a las operaciones con respuesta multi-frame de mayor tamaño (\texttt{probe\_pids} y \texttt{snapshot}), mientras que las consultas individuales como \texttt{uds\_read\_did} se mantienen en una mediana de 98\,ms.

% Cadencia real medida con scripts/db_metrics.py sobre las sesiones de monitorización
% continua de diagnostics.db (intervalo entre muestras consecutivas de cada PID).
En el modo de monitorización continua, el periodo de ciclo real medido escala de forma lineal con el número de PIDs solicitados concurrentemente, debido al coste acumulado de las transacciones ISO-TP secuenciales en el bus CAN. Cuando el cliente solicita un conjunto reducido de 7 PIDs (configurado con un intervalo objetivo de 500\,ms), el sistema estabiliza su periodo de refresco con una mediana de 586\,ms y una media de 591\,ms, reflejando un overhead de procesamiento e I/O de aproximadamente 80--100\,ms respecto al temporizador ideal. A medida que el volumen de PIDs asciende a 18 y 24 solicitudes por ciclo, la naturaleza síncrona y bloqueante del escaneo expande el periodo de ciclo a medianas de 703\,ms y 816\,ms, respectivamente. En todos los escenarios concurrentes analizados bajo bus determinista, la dispersión estadística se mantuvo notablemente acotada (con rangos intercuartílicos inferiores a 25\,ms), garantizando una tasa de actualización altamente predecible y libre de pérdidas de muestras o saturación en el hilo de transmisión BLE.

\begin{table}[H]
\caption{Cadencia real de la monitorización continua medida sobre la base de datos SQLite (intervalo medido entre ráfagas consecutivas de actualización).}
\label{tab:cadencia}
\centering
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{PIDs por ciclo} & \textbf{Periodo mediana (ms)} & \textbf{Media (ms)} & \textbf{P10 -- P90 (ms)} \\
\hline
7  & 586 & 591 & 571 -- 631 \\
18 & 703 & 706 & 692 -- 713 \\
24 & 816 & 821 & 806 -- 838 \\
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[height=9.5cm]{Ilustraciones/iter_3_ard.png}
\subcaption{Consola del simulador Arduino.}
\label{fig:res_consola_ard}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
\includegraphics[height=9.5cm]{Ilustraciones/iter_3_pi.png}
\subcaption{Consola de la Raspberry Pi.}
\label{fig:res_consola_pi}
\end{minipage}
\caption{Traza de ejecución bajo condiciones de bus realistas: consola del simulador Arduino emitiendo tramas de difusión (\texttt{[BC TX]}) e inyectando DTCs por interrupción hardware (a) y consola de la Raspberry Pi decodificando respuestas OBD-II y UDS (b).}
...
\begin{figure}[H]
\centering
\includegraphics[height=9.5cm]{Ilustraciones/datos_real.png}%
\hspace{0.04\textwidth}%
\includegraphics[height=9.5cm]{Ilustraciones/uds_real.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=9.5cm]{Ilustraciones/dtcs_real.png}%
\hspace{0.04\textwidth}%
\includegraphics[height=9.5cm]{Ilustraciones/sesiones_sim.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=9.5cm]{Ilustraciones/sesion_detalle.png}%
\hspace{0.04\textwidth}%
\includegraphics[height=9.5cm]{Ilustraciones/logs_real.png}
...
\hspace{0.04\textwidth}%
\includegraphics[height=9.5cm]{Ilustraciones/ajustes.png}
\caption{Pantalla \textit{Conexión} (izquierda), con la gestión del enlace BLE, y pantalla \textit{Ajustes} (derecha), con el modo de simulación y preferencias.}
