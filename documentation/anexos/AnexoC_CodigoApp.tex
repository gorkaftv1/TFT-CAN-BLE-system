\chapter*{Anexo C — Código fuente de la aplicación móvil}
\addcontentsline{toc}{chapter}{Anexo C — Código fuente de la aplicación móvil}
\label{anexo:app}
...
private async writeRx(data: string, withResponse = true): Promise<void> {
  ...
  // Fragmentar en chunks de MTU bytes para el caso de payloads grandes
  for (let offset = 0; offset < bytes.length; offset += MTU) {
    const slice = bytes.subarray(offset, offset + MTU);
    let bin = '';
